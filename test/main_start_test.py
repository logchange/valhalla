...
class MainStartTest(unittest.TestCase):
    ...
    def test_start_exits_when_version_empty(self):
        # given: version_to_release is empty both before and after from_config
        mock_vtr = MagicMock()
        mock_vtr.is_version_empty.side_effect = [True, True]
        ...
        with patch('valhalla.main.__version_to_release', return_value=mock_vtr), \
             ...
             patch('valhalla.extends.valhalla_extends.resolve', side_effect=lambda s: s), \
             patch('valhalla.main.error') as mock_error:
            # when/then: start should exit with code -1 and log an error
            ...
            self.assertTrue(any(
                call.args and isinstance(call.args[0], str) and call.args[0].startswith("Version to release is empty")
